...
mul = 2*np.pi/10 # mul = 2*pi/period 
m = 2
d = 25
...
def bigxi2(ht):
    h1 = mul*ht[:,0]
    h2 = mul*ht[:,1]
    return tf.transpose(tf.constant([np.ones(ht.shape[0]), np.cos(h1), np.sin(h1), np.cos(2*h1), np.sin(2*h1),
                                     np.cos(h2), np.cos(h1)*np.cos(h2), np.sin(h1)*np.cos(h2), np.cos(2*h1)*np.cos(h2), np.sin(2*h1)*np.cos(h2),
                                     np.sin(h2), np.cos(h1)*np.sin(h2), np.sin(h1)*np.sin(h2), np.cos(2*h1)*np.sin(h2), np.sin(2*h1)*np.sin(h2),
                                     np.cos(2*h2), np.cos(h1)*np.cos(2*h2), np.sin(h1)*np.cos(2*h2), np.cos(2*h1)*np.cos(2*h2), np.sin(2*h1)*np.cos(2*h2),
                                     np.sin(2*h2), np.cos(h1)*np.sin(2*h2), np.sin(h1)*np.sin(2*h2), np.cos(2*h1)*np.sin(2*h2), np.sin(2*h1)*np.sin(2*h2)]
                                     ,dtype = 'float32'))
...
def backwardode(mus, s):
    ...
    dh4 = tf.stack([-2*tf.math.sin(2*h1),tf.zeros((n_instances,))],axis =1)
    dh5 = tf.stack([2*tf.math.cos(2*h1),tf.zeros((n_instances,))],axis =1)
    
    dh6 = tf.stack([tf.zeros((n_instances,)),-tf.math.sin(h2)],axis = 1)
    dh7 = tf.stack([-tf.math.sin(h1)*tf.math.cos(h2),-tf.math.cos(h1)*tf.math.sin(h2)],axis =1)
    dh8 = tf.stack([tf.math.cos(h1)*tf.math.cos(h2),-tf.math.sin(h1)*tf.math.sin(h2)],axis =1)
    dh9 = tf.stack([-2*tf.math.sin(2*h1)*tf.math.cos(h2),-tf.math.cos(2*h1)*tf.math.sin(h2)],axis =1)
    dh10 = tf.stack([2*tf.math.cos(2*h1)*tf.math.cos(h2),-tf.math.sin(2*h1)*tf.math.sin(h2)],axis =1)
    
    dh11 = tf.stack([tf.zeros((n_instances,)),tf.math.cos(h2)],axis = 1)
    dh12 = tf.stack([-tf.math.sin(h1)*tf.math.sin(h2),tf.math.cos(h1)*tf.math.cos(h2)],axis =1)
    dh13 = tf.stack([tf.math.cos(h1)*tf.math.sin(h2),tf.math.sin(h1)*tf.math.cos(h2)],axis =1)
    dh14 = tf.stack([-2*tf.math.sin(2*h1)*tf.math.sin(h2),tf.math.cos(2*h1)*tf.math.cos(h2)],axis =1)
    dh15 = tf.stack([2*tf.math.cos(2*h1)*tf.math.sin(h2),tf.math.sin(2*h1)*tf.math.cos(h2)],axis =1)
    
    
    dh16 = tf.stack([tf.zeros((n_instances,)),-2*tf.math.sin(2*h2)],axis =1)
    dh17 = tf.stack([-tf.math.sin(h1)*tf.math.cos(2*h2),-2*tf.math.cos(h1)*tf.math.sin(2*h2)],axis =1)
    dh18 = tf.stack([tf.math.cos(h1)*tf.math.cos(2*h2),-2*tf.math.sin(h1)*tf.math.sin(2*h2)],axis =1)
    dh19 = tf.stack([-2*tf.math.sin(2*h1)*tf.math.cos(2*h2),-2*tf.math.cos(2*h1)*tf.math.sin(2*h2)],axis =1)
    dh20 = tf.stack([2*tf.math.cos(2*h1)*tf.math.cos(2*h2),-2*tf.math.sin(2*h1)*tf.math.sin(2*h2)],axis =1)
    
    
    dh21 = tf.stack([tf.zeros((n_instances,)),2*tf.math.cos(2*h2)],axis = 1)
    dh22 = tf.stack([-tf.math.sin(h1)*tf.math.sin(2*h2),2*tf.math.cos(h1)*tf.math.cos(2*h2)],axis =1)
    dh23 = tf.stack([tf.math.cos(h1)*tf.math.sin(2*h2),2*tf.math.sin(h1)*tf.math.cos(2*h2)],axis =1)
    dh24 = tf.stack([-2*tf.math.sin(2*h1)*tf.math.sin(2*h2),2*tf.math.cos(2*h1)*tf.math.cos(2*h2)],axis =1)
    dh25 = tf.stack([2*tf.math.cos(2*h1)*tf.math.sin(2*h2),2*tf.math.sin(2*h1)*tf.math.cos(2*h2)],axis =1)
    
    
    
    bigxip = mul*tf.stack([dh1,dh2,dh3,dh4,dh5,dh6,dh7,dh8,dh9,dh10,dh11,dh12,dh13,dh14,dh15,
                           dh16,dh17,dh18,dh19,dh20,dh21,dh22,dh23,dh24,dh25],axis = 1)
